...
describe('awarenessText', () => {
  it('states a real email count when both calendar and inbox are connected', () => {
    ...
    expect(result).toContain('14 emails');
  ...
  it('uses the singular form for exactly one email', () => {
    ...
    expect(result).toContain('1 email');
    expect(result).not.toContain('1 emails');
  });

  it('says "your inbox" rather than the awkward "0 emails" when the inbox is connected but genuinely empty — found live, 20 July 2026: Awareness previously claimed to have been watching with nothing to show for it', () => {
    const result = awarenessText(false, true, 0);
    expect(result).toContain('your inbox');
    expect(result).not.toContain('0 emails');
  });
  ...
  it('never states a conclusion about whether anything qualified for attention — that remains Intervention\'s job entirely, and stating it here could directly contradict whatever Intervention shows immediately below', () => {
    const result = awarenessText(true, true, 14);
